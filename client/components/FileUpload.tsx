...
const FileUpLoader: React.FC<FileUpLoaderProps> = ({ name }) => {
  ...
  const onFileDrop = useCallback(
    async (e: React.SyntheticEvent<EventTarget>) => {
      ...
      formData.append('upload_preset', 'coinplus-webapp');

      store.setUploadingImage(true);
      const data = await fetch('https://api.cloudinary.com/v1_1/aramix/image/upload', {
        method: 'POST',
        body: formData,
      })
        .then((res) => {
      ...
      render={({ field: { name, onBlur, ref } }) => (
        <>
          <div className="mb-2 flex justify-between items-center">
            ...
            <div>{store.uploadingImage && <Spinner color="text-yellow-400" />}</div>
          </div>
          <p className={`text-red-500 text-xs italic mb-2 ${errors[name] ? 'visible' : 'invisible'}`}>
